...
from FLAlgorithms.optimizers.fedoptimizer import FedmirrorOptimizer
from FLAlgorithms.users.userbase import User
import copy

# Implementation for pFeMe clients

class UserFedmirror(User):
    def __init__(self, device, numeric_id, train_data, test_data, model, batch_size, learning_rate,beta,lamda,
        ...
        self.optimizer = FedmirrorOptimizer(self.model.parameters(), lr=self.personal_learning_rate, lamda=self.lamda)
    ...
    def set_parameters_mirror_personal(self, model):
    ...
    def train(self, epochs):
        ...
        for epoch in range(1, self.local_epochs + 1):  # local update
            ...
            #if epoch == 1:
            #    persionalized_model, _ = self.optimizer.step(self.regularizer, self.lamdaCO,self.modeltype,self.l1const,self.l2const)
            #else:
            self.optimizer.step(self.regularizer, self.lamdaCO,self.modeltype,self.l1const,self.l2const)
