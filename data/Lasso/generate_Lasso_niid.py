...
def generate_Lasso_niid(d1, d0):
    ...
    for i in range(NUM_USER):
        mean_x[i] = np.random.normal(0, 1, dimension)
        xx = np.random.multivariate_normal(mean_x[i], cov_x, samples_per_user[i])
        yy = np.zeros(samples_per_user[i])
        w=np.zeros(1024-d1)
        w[np.random.choice(1024-d1,d0,replace=False)]=1
        ww= np.concatenate((np.ones(d1),np.zeros(1024-d1)), axis=None)+np.concatenate((np.zeros(d1),w), axis=None)
...
def main():
    ...
    X, y, W = generate_Lasso_niid(32,32) # synthetic (0.5, 0.5)
