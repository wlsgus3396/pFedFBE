...
def read_user_data(index,data,dataset):
    ...
    if(dataset == "Mnist"):
        ...
        X_train, y_train, W_train, X_test, y_test, W_test = train_data['x'], train_data['y'],train_data['W'], test_data['x'], test_data['y'],test_data['W']
        X_train = torch.Tensor(X_train).type(torch.float32)
        y_train = torch.Tensor(y_train).type(torch.float64)
        W_train= torch.Tensor(W_train).type(torch.float32)
        X_test = torch.Tensor(X_test).type(torch.float32)
        y_test = torch.Tensor(y_test).type(torch.float64)
        W_test= torch.Tensor(W_test).type(torch.float32)
        train_data = [(x, y,w) for x, y,w in zip(X_train, y_train,W_train)]
        test_data = [(x, y,w) for x, y,w in zip(X_test, y_test,W_test)]
    elif(dataset=="Matrix"):
        X_train, y_train, W_train, X_test, y_test, W_test = train_data['x'], train_data['y'],train_data['W'], test_data['x'], test_data['y'],test_data['W']
        X_train = torch.Tensor(X_train).type(torch.float32)
        y_train = torch.Tensor(y_train).type(torch.float64)
